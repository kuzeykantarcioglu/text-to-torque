...
We compare six termination settings:
\begin{itemize}
    ...
    \item \textbf{Adaptive calibration}: an automatic threshold search that starts from a tight threshold, trains a short stage, checks episode length and termination count, and relaxes to the next threshold only if the current stage fails.
\end{itemize}
...
\subsection{Reference Repair}

We also test a simple reference-conditioning intervention on the hardest seed-0 squat case. Before converting the generated CSV to an NPZ reference, we either prepend a 0.5 s hold of the initial pose, apply a width-7 moving average to root/joint trajectories, or do both. This tests whether the strict squat failure is mainly caused by initial-state jerk or high-frequency generated-reference artifacts.
...
\subsection{Adaptive Calibration and Reference Repair}

The calibrated-threshold result still uses manual inspection of the gradual curriculum. We therefore added an adaptive threshold-calibration run that tries thresholds $0.5, 1, 2$ in order. Each stage trains for 400 iterations and passes only if mean episode length is at least 220 and final termination total is at most 1. Table~\ref{tab:adaptive_calibration} shows that threshold 0.5 is too strict, threshold 1 is close but still fails the gate, and threshold 2 passes with full-horizon episodes and zero final terminations. This converts the manual calibrated-threshold result into a simple automatic procedure.

\begin{table}[t]
\centering
\caption{Adaptive threshold calibration for seed-0 squat. The procedure automatically rejects thresholds 0.5 and 1, then selects threshold 2.}
\label{tab:adaptive_calibration}
\resizebox{\linewidth}{!}{
\begin{tabular}{lrrrrr}
\toprule
Stage & Reward & Episode length & Body pos. err. & Joint pos. err. & Term. total \\
\midrule
Threshold 0.5 & 1.954 & 89.31 & 0.455 & 2.005 & 13.250 \\
Threshold 1 & 6.477 & 219.65 & 0.461 & 1.828 & 1.375 \\
Threshold 2 & 10.594 & 250.00 & 0.260 & 1.966 & 0.000 \\
\bottomrule
\end{tabular}}
\end{table}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{final_adaptive_calibration_curves.png}
    \caption{Adaptive calibration curves. The metric gate rejects over-tight thresholds and accepts threshold 2.}
    \label{fig:adaptive_calibration}
\end{figure}

We also tested whether a trivial repair of the generated reference could rescue strict training. Table~\ref{tab:reference_repair} and Figure~\ref{fig:reference_repair} show that an initial hold, smoothing, and hold-plus-smoothing all remain near the original strict failure: episode lengths stay near 13 steps and termination counts remain very high. This negative result is useful because it rules out a simple explanation based only on initial-reference discontinuity or high-frequency CSV noise.

\begin{table}[t]
\centering
\caption{Strict seed-0 squat with simple generated-reference repairs. Initial hold and smoothing do not rescue the strict-termination failure.}
\label{tab:reference_repair}
\resizebox{\linewidth}{!}{
\begin{tabular}{lrrrrr}
\toprule
Condition & Reward & Episode length & Body pos. err. & Joint pos. err. & Term. total \\
\midrule
Strict original & 0.837 & 13.52 & 0.234 & 0.484 & 144.875 \\
Hold repair & 0.767 & 12.88 & 0.231 & 0.471 & 144.458 \\
Smooth repair & 0.760 & 12.66 & 0.231 & 0.475 & 145.333 \\
Hold + smooth repair & 0.829 & 13.14 & 0.231 & 0.487 & 146.583 \\
\bottomrule
\end{tabular}}
\end{table}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{final_reference_repair_curves.png}
    \caption{Reference-repair ablation under strict termination. Simple CSV conditioning does not move the policy out of the early-termination regime.}
    \label{fig:reference_repair}
\end{figure}
