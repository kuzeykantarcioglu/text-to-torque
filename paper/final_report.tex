...
The main empirical result is that termination design exposes a clear failure boundary. Strict termination works well for gesture-like motions: wave and tap-head policies reach near-full episode lengths and final rewards around 18 under strict thresholds. Walking is also learnable under strict termination, reaching final reward 13.12 and mean episode length 244.78 out of 250. In contrast, strict termination fails severely on the squat motion, which has the largest root-height range among our references. The loose squat policy reaches final reward 13.66 and full 250-step episodes, while the strict squat policy reaches only reward 0.84 and terminates after 13.52 steps on average, with a very large termination count. This shows why tracking error must be interpreted together with episode length: strict squat reports relatively low tracking error only because it evaluates a short early segment before termination.

The curriculum experiment is a useful negative result. Our two-stage walk curriculum successfully resumes training from a loose first stage into a strict second stage, but it does not outperform strict training from scratch. This suggests that relaxed termination is not automatically beneficial; it is most useful when strict termination prevents full-horizon exploration, as in the squat case. Overall, the project provides a small but complete empirical study of the gap between text-generated kinematic motion and physically executable humanoid control. The strongest conclusion is that generated-reference diagnostics and termination statistics can reveal when PPO tracking failure is caused not by PPO alone, but by the interaction between generated motion properties and the episode termination rule.
